...
function initProposalInteraction() {
    ...
    // Reference to background music (added in game.js)
    let backgroundMusic = document.getElementById('backgroundMusic') || document.querySelector('audio');
    // Add proposal music element if it doesn't exist
    ...
    if (!proposalMusic) {
      ...
      proposalMusic.src = 'audio/proposal-song.mp3'; // Path to your romantic song
    ...
    yesButton.addEventListener('click', () => {
      ...
      if (backgroundMusic) {
        ...
        // Just play proposal music if no background music found
        proposalMusic.play().catch(err => console.error("Could not play proposal music:", err));
      }
    ...
    function applyCyberpunkStyling() {
      ...
      // Add photo frame to celebration if not present
      if (celebration && !celebration.querySelector('.photo-frame')) {
        const photoFrame = document.createElement('div');
        photoFrame.className = 'photo-frame';
        celebration.appendChild(photoFrame);
      }
    }
    ...
    function createEnhancedCelebration() {
      ...
      // Create photo memory effect
      setTimeout(() => {
        const photoFrame = celebration.querySelector('.photo-frame');
        if (photoFrame) {
          photoFrame.style.animation = 'pulse 2s infinite';
        }
      }, 1000);
    }
